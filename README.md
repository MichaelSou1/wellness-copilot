...
Wellness Copilot 是一个面向个人健康管理的 LangGraph 多 Agent 系统。它不是“把问题丢给一个 LLM 回答”的聊天 Demo，而是把多专家协作、长期记忆、RAG 证据检索、通用多模态图片理解、结构化日志、真实提醒、Apple Calendar 日程写入、微信入口、FastAPI 后端、SQLite 任务队列、Outbox、Prometheus 可观测性、备份和 Docker 常驻部署串成一个可实际运行的健康助手。
...
>
> 2026-05-30 情景记忆 RAG 个性化归因 & MIN_COUNT 调优：项目有两套独立 RAG（KB pull-RAG 喂通识 / per-user 情景记忆 push-RAG 喂个性化）。两臂因果实验证明 personalization 基线 3.70 偏低的主因不是模型织入弱，而是评测从不给用户种历史 episode、情景记忆 push-RAG 开火率仅 9.4%；种入历史后开火率→100%、其针对类别 profile_personalization +1.25。情景记忆检索本身 top-1 相关率 90.6% / MRR 0.945、下游采纳率 90.6%（与 KB RAG 同档）。据"个性化收益在第 3 条历史即吃满、且浅历史检索反而更准"把放行门槛 `EPISODE_SEMANTIC_MIN_COUNT` 从 8 下调到 5，让用户早 3 轮获得跨会话个性化。
...
2026-05-29 后端化升级新增实测亮点（`reports/backend_upgrade_report_20260529-015011.md`、`reports/backend_upgrade_report_20260529-015243.md`、`reports/backend_upgrade_report_20260529-015629.md`）：

- **Agent 服务化与流式可演示**：新增 FastAPI 后端，提供 `/healthz`、`/v1/chat`、`/v1/chat/stream`、`/v1/jobs`、`/metrics`。真实 smoke test 覆盖同步回答、SSE 节点事件和异步 job 轮询，全部通过；`/v1/chat/stream` 能看到 `InputAccumulator -> TurnStart -> QueryRewriter -> MultiModalPreprocessor -> Orchestrator` 等节点事件。
- **同步长任务自动降级为异步 job**：`/v1/chat` 先创建并领取同一个 SQLite job，超过 `BACKEND_SYNC_TIMEOUT_SEC` 后返回 `job_id`，后台继续执行，避免 HTTP 请求被长推理拖死。真实 chat 压测 10/10 成功，其中 2/10 在 35s 同步超时阈值处转为异步；整组 p50 1.93s、p95 35.02s。
- **SQLite Job Queue + Outbox 可靠执行链路**：新增 `agent_jobs` 和 `outbox_events`，支持 lease 临时锁、`idempotency_key` 幂等、失败后 30s / 60s / 120s 固定重试，超过 3 次进入 dead。队列单测覆盖重试间隔、lease 过期重领和 outbox 幂等，`tests/test_backend_queue.py` 3/3 通过。
- **背压与队列瓶颈可量化**：`BACKEND_MAX_PENDING_JOBS` / `BACKEND_MAX_RUNNING_JOBS` 控制入口背压，队列深度和 queue lag 暴露到报告和 Prometheus。单 worker 真实 `/v1/jobs` 压测 8/8 成功，端到端 p50 28.32s、p95 65.73s，queue lag p50 26s、p95 42s，说明瓶颈已能被定位为 worker 并发不足，而不是 API/SQLite 入口失败。
...
2026-05-30 情景记忆 RAG 个性化归因 & MIN_COUNT 调优（`reports/rag_diagnosis_phase12_findings.md`，独立 judge，53 样本 × 多臂实跑）：

- **把“personalization 偏低”做成可证伪的因果归因，而非直接归咎模型**：项目有两套互相独立的 RAG——KB pull-RAG（领域通识，喂 completeness/safety）与 per-user 情景记忆 push-RAG（FAISS，喂 personalization）。原 eval 每条样本造全新冷启动用户、从不种历史 episode，叠加放行门槛 `MIN_COUNT=8`，导致情景记忆 push-RAG 在评测里**开火率仅 9.4%**（且全是多轮第 2 轮的同线程最近记忆，语义跨会话检索 0 次）。新增 `EVAL_SEED_EPISODES` 种子化 + 两臂对照（control 无历史 vs treatment 每用户种 9 条带 relevance 标注的 episode，同 judge 背靠背），证明 3.70 的主因是 **b1（情景记忆 RAG 空转，属评测装置缺陷）** 而非模型织入弱：开火率 9.4%→100%，个性化在其针对类别 profile_personalization **+1.25**、multi_turn +0.80、progress_review +0.50 提升，而自洽单轮类别（doctor/multi_expert/chitchat）零变化——机制自洽，说明 3.70 实则低估了线上老用户的真实个性化。
- **情景记忆 RAG 自身检索质量被独立量化，与 KB RAG 同档优秀**：53 个带标注场景（每条 3 相关 + 6 干扰）上 `retrieve_similar` top-1 相关率 **90.6%**、**MRR 0.945**、recall@3 0.79 / recall@5 0.92 / hit@3 0.98；下游采纳率 **90.6%**（注入的 `episode_context` 独有内容确被织进答案），证明 retrieve→inject→adopt 三段全通，瓶颈从来不是检索，而是它“不开火”。
- **用数据把放行门槛 `EPISODE_SEMANTIC_MIN_COUNT` 从 8 下调到 5**：历史深度 sweep 显示浅历史检索反而更准（recall@3：深度 3=1.00 / 5=0.88 / 8=0.81），个性化收益在闸门一开（深度 3）即吃满约 83%（+0.42 / 最终 +0.50），8→9 落在噪声内；据此把默认 8→5，让用户**早 3 轮**获得跨会话个性化且检索质量无损。
- **可复现工具链与实跑加固**：`scripts/generate_episode_seeds.py`（LLM 合成带 relevance 标注的 episode 历史，分片并行 + 增量落盘）、`scripts/evaluate_episode_memory.py`（recall@k/MRR/采纳率/深度 sweep）、`scripts/compare_output_reports.py`（两臂逐样本配对 Δ）；并定位修复了实跑时上游代理 keep-alive 半死连接拖垮长驻进程的卡死（`LLM_DISABLE_KEEPALIVE` + 请求级超时 + 逐样本落盘续跑），53×2 全过、0 卡死。
...
端到端评测专门设置了 17 个需要画像转成量化建议的检查点，deterministic 检查 17/17 通过；profile echo 边界样本 4/4 通过，避免模型把画像信息生硬复述给用户。LLM-as-Judge 个性化维度基线为 3.70/5 —— 2026-05-30 的两臂因果实验进一步把这个低分查清楚了：它主因是**评测装置缺陷**而非模型织入弱。评测里每条样本都是零历史的冷启动用户，喂个性化的那套 per-user 情景记忆 push-RAG 因放行门槛未达标几乎从不触发（开火率仅 9.4%）；给用户种入历史后，该维度在其针对类别 profile_personalization 提升 +1.25、push-RAG 开火率→100%，并据此把门槛 `EPISODE_SEMANTIC_MIN_COUNT` 从 8 下调到 5（详见上方技术亮点 2026-05-30 块与 `reports/rag_diagnosis_phase12_findings.md`）。也就是说 3.70 低估了线上老用户的真实个性化能力，而非“模型不会用画像”。
...
Docker Compose 提供五个长期运行服务：

- `api`：FastAPI 后端服务，提供同步聊天、SSE 流式聊天、异步 job 和 `/metrics`。
- `worker`：微信长轮询，下载图片、聚合碎片输入，把完整输入写入 `agent_jobs`，不阻塞运行完整 Agent。
- `agent-worker`：消费 `agent_jobs`，调用 LangGraph，并把微信回复写入 outbox。
- `dispatcher`：扫描 due reminders 和 outbox，到点推送微信或执行外部 side effect；失败按 30s / 60s / 120s 重试。
...
后端化测评进一步把入口服务、队列和 Agent 内部耗时拆开：真实 `/v1/chat` 压测 10/10 成功，p50 1.93s、p95 35.02s；真实 `/v1/jobs` 压测 8/8 成功，p50 28.32s、p95 65.73s，queue lag p50 26s、p95 42s。这个结果说明 FastAPI/SQLite 入口本身稳定，单 worker 场景的异步延迟主要来自 worker 排队和 Agent 推理。
...
### 3. 可选：启动 FastAPI 后端服务

后端 MVP 把原来的 CLI/微信脚本入口包装成 HTTP API，同时保留 LangGraph 核心编排不变。
...
curl -N -X POST http://127.0.0.1:8000/v1/chat/stream \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $BACKEND_API_KEY" \
  -d '{"user_id":"demo","message":"帮我安排一个轻量训练计划。"}'
```
...
curl -X POST http://127.0.0.1:8000/v1/jobs \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $BACKEND_API_KEY" \
  -d '{"user_id":"demo","message":"给我一句恢复建议。"}'
python scripts/agent_job_worker.py --once
```

真实 API smoke 和 fake agent 压测：

```bash
python scripts/backend_smoke.py --run-worker-once
FAKE_AGENT_MODE=1 uvicorn wellness_copilot.api:app --host 127.0.0.1 --port 8000
python scripts/load_backend.py --requests 100 --concurrency 10
```

分层业务样本压测：

```bash
python scripts/load_backend.py \
  --dataset eval/backend_load_dataset.jsonl \
  --requests 20 \
  --concurrency 3 \
  --mode chat \
  --shuffle \
...
### 5. 可选：启用 Apple Calendar / iCloud CalDAV
...
启动微信 worker、Agent job worker 和 outbox dispatcher：

```bash
python scripts/wechat_ilink_worker.py
python scripts/agent_job_worker.py
...
docker compose logs -f api worker agent-worker dispatcher backup
```
...
```bash
docker compose exec api python scripts/backend_smoke.py --base-url http://127.0.0.1:8000 --run-worker-once
docker compose exec api python scripts/load_backend.py --base-url http://127.0.0.1:8000 --requests 100 --concurrency 10
docker compose exec dispatcher python scripts/outbox_dispatcher.py --once --dry-run
```
...
```mermaid
flowchart TD
    API[FastAPI / CLI] --> IA[InputAccumulator<br/>微信碎片输入聚合]
    WX[微信文本/图片] --> Worker[scripts/wechat_ilink_worker.py<br/>长轮询 Worker]
    Worker --> Jobs[(agent_jobs<br/>SQLite Job Queue)]
    Jobs --> AgentWorker[scripts/agent_job_worker.py]
    AgentWorker --> IA
    ...
    ORC -->|DIRECT| FINAL
    FINAL --> API
    FINAL --> Outbox[(outbox_events)]
    Outbox --> OutboxDisp[scripts/outbox_dispatcher.py]
    OutboxDisp --> Worker
    Worker --> WX
...
| `wellness_copilot/api.py` | FastAPI 后端入口，同步聊天、SSE、job、metrics。 |
| `wellness_copilot/backend_runtime.py` | 统一 Agent turn runtime，供 API/CLI/job worker 复用。 |
| `wellness_copilot/backend_queue.py` | SQLite job queue 和 outbox。 |
| `wellness_copilot/backend_outbox.py` | outbox side effect 投递。 |
...
| `scripts/agent_job_worker.py` | 异步消费 Agent job。 |
| `scripts/outbox_dispatcher.py` | 到点提醒和微信回复 outbox dispatcher。 |
| `scripts/backend_dead_jobs.py` | 排查 dead job / dead outbox。 |
| `scripts/build_backend_load_dataset.py` | 从端到端评测集构建后端压测 JSONL。 |
| `scripts/backend_smoke.py` / `scripts/load_backend.py` | 后端 smoke test 与压测报告。 |
| `scripts/backup_loop.py` | SQLite/JSON/reports 备份，可选 OSS 上传。 |
| `docker-compose.yml` | api / worker / agent-worker / dispatcher / backup 常驻部署。 |
...
| `WECHAT_*` | 否 | 微信入口和主动推送。 |
| `BACKEND_*` | 否 | FastAPI 鉴权、SQLite queue/outbox、lease 与重试间隔。 |
| `FAKE_AGENT_MODE` | 否 | 后端压测时模拟 Agent，避免消耗真实 LLM。 |
...
| `uvicorn wellness_copilot.api:app` | 是 | `LLM_*` + `ORCHESTRATOR_LLM_*`；图片轮按需 `MULTIMODAL_LLM_*`；后端配置用 `BACKEND_*` | FastAPI 后端。`/v1/chat` 同步运行 Agent，超时后返回 job；`/v1/chat/stream` 输出 SSE；`/v1/jobs` 只入队。 |
| `scripts/agent_job_worker.py` | 是 | `LLM_*` + `ORCHESTRATOR_LLM_*`；图片轮按需 `MULTIMODAL_LLM_*`；`BACKEND_PREWARM_RAG` 可预热 RAG | 消费 `agent_jobs`，调用 LangGraph，把微信回复等 side effect 写入 outbox。 |
| `scripts/wechat_ilink_worker.py` | 否 | `WECHAT_*` + `BACKEND_*` | 微信长轮询 worker，下载媒体、聚合碎片输入，并写入 SQLite job queue；不直接调用 LangGraph。 |
| `scripts/outbox_dispatcher.py` | 否 | SQLite / `WECHAT_*` / `ICLOUD_*` | 投递 `outbox_events` 和 due reminders；失败按后端重试策略进入 retry/dead。 |
| `scripts/backend_smoke.py` | 是 | `BACKEND_API_KEY`；目标 API 进程使用 `LLM_*` | 调 `/healthz`、`/v1/chat`、SSE、`/v1/jobs` 做后端真实链路 smoke。 |
| `scripts/load_backend.py` | 可选 | `BACKEND_API_KEY`；真实 Agent 用 `LLM_*`，`FAKE_AGENT_MODE=1` 时不消耗 LLM | 后端压测脚本，支持 chat/jobs、分层数据集、shuffle、p50/p95/queue lag 报告。 |
...
| `scripts/reminder_dispatcher.py` | 否 | SQLite / WeChat push | 旧版到点提醒派发；当前常驻部署优先使用 `scripts/outbox_dispatcher.py`。 |
...
| `scripts/evaluate_episode_memory.py` | 否 | `RAG_*` / episode 配置 | 情景记忆 RAG 专项评测：`retrieve_similar` 的 recall@k/MRR/采纳率 + 历史深度 sweep，只用本地 embedding。 |
| `scripts/generate_episode_seeds.py` | 是 | `LLM_*` | 用 LLM 合成带 relevance 标注的 per-user episode 历史（`eval/episode_seeds.jsonl`），供 output/episode 评测种子化。 |
| `scripts/compare_output_reports.py` | 否 | 无 | 两份 output-eval 报告按样本配对，输出各维度与 personalization 的 Δ。 |
...
```env
BACKEND_API_KEY=
BACKEND_DB_PATH=health_logs.db
BACKEND_JOB_LEASE_SEC=300
BACKEND_OUTBOX_LEASE_SEC=120
BACKEND_SQLITE_BUSY_TIMEOUT_MS=10000
BACKEND_SYNC_TIMEOUT_SEC=35
BACKEND_MAX_PENDING_JOBS=50
BACKEND_MAX_RUNNING_JOBS=8
BACKEND_PREWARM_RAG=false
BACKEND_PREWARM_RAG_QUERY=健康建议
BACKEND_RETRY_DELAYS_SEC=30,60,120
FAKE_AGENT_MODE=false
FAKE_AGENT_DELAY_MS=150
```

语义说明：

- `BACKEND_SYNC_TIMEOUT_SEC`：同步 `/v1/chat` 最多等待多久；超时后返回 `job_id`，后台继续执行同一个 job。
- `BACKEND_JOB_LEASE_SEC` / `BACKEND_OUTBOX_LEASE_SEC`：worker 领取任务后的临时锁时间，崩溃后锁过期可重领。
- `BACKEND_MAX_PENDING_JOBS` / `BACKEND_MAX_RUNNING_JOBS`：简单背压阈值，避免入口无限堆积。
- `BACKEND_RETRY_DELAYS_SEC=30,60,120`：失败后第 1/2/3 次重试分别等待 30 秒、1 分钟、2 分钟，再失败进入 dead。
- `BACKEND_PREWARM_RAG`：API / worker 启动时预热各专家知识库，牺牲冷启动时间换取首个真实请求稳定性。
...
- `reports/rag_eval_report_20260521-175205.json`
...
| 服务 | 命令 | 作用 |
|---|---|---|
| `api` | `uvicorn wellness_copilot.api:app` | FastAPI/SSE、异步 job、Prometheus `/metrics`。 |
| `worker` | `python scripts/wechat_ilink_worker.py` | 接收微信消息，聚合碎片输入，写入 SQLite job queue。 |
| `agent-worker` | `python scripts/agent_job_worker.py` | 消费 job，调用 graph，生成最终回复。 |
| `dispatcher` | `python scripts/outbox_dispatcher.py` | 扫描 reminders/outbox，到点主动推送或投递外部 side effect。 |
...
# 后端真实链路 smoke / 压测
python scripts/backend_smoke.py --base-url http://127.0.0.1:8000 --run-worker-once
python scripts/load_backend.py --base-url http://127.0.0.1:8000 --dataset eval/backend_load_dataset.jsonl --requests 20 --concurrency 3 --mode chat --shuffle --per-category 2
python scripts/load_backend.py --base-url http://127.0.0.1:8000 --dataset eval/backend_load_dataset.jsonl --requests 12 --concurrency 3 --mode jobs --poll-timeout 240 --shuffle --per-category 2
```
...
- `reports/output_eval_report_20260521-193421.json`
- `reports/rag_eval_report_20260521-175205.json`
