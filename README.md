...
flowchart TD
    ...
    subgraph Dispatcher [Dispatcher · 子 agent 调度 · 并行 fan-out]
      ...
      Psychologist[Psychologist<br/>ReAct + RAG 按需]
      Doctor[Doctor<br/>ReAct + medical MCP]
    end
    ...
    class Judge meta;
    class Trainer,Nutritionist,Psychologist,Doctor expert;
    class Scratchpad,History,EpStore,SafetyKB,Ctx store;
```

> 实线 = 控制流；虚线 = 状态侧通道。普通寒暄、能力介绍和画像更新由 **Orchestrator** 直接回答；训练、营养、身心恢复、医学建议等专业请求由 Orchestrator 通过 tool call 调用对应子 agent。同一批计划内的多专家通过 ThreadPoolExecutor 并行执行。Replan 路径**直接 Dispatcher/Critic → Orchestrator**，不再经过 TurnStart / QueryRewriter——否则会清掉本轮已积累的 plan/scratchpad，且 contextualized_query 在同一用户轮内必须稳定。
...
| **Orchestrator** | 主 Agent / LangGraph 节点 | 直接处理寒暄、澄清、画像更新、能力介绍；专业请求以 tool call 形式调用 `Trainer/Nutritionist/Psychologist/Doctor`；routing message 注入**用户画像摘要** + **近期情节记忆**；replan 模式追加专家时不重复已执行角色 |
...
| **Psychologist** *(子 agent)* | Dispatcher 调用的 callable | `retrieve_psychologist_knowledge`（按需）+ 画像读写；profile 字段白名单只含 age/injuries + 完整 mental_state |
| **Doctor** *(子 agent)* | Orchestrator tool call / Dispatcher callable | 提供一般医学信息、症状风险分层、就医建议和用药/处方边界；接入 **medical-mcp**（FDA/PubMed/WHO/RxNorm 等）；被调用后的最终回答必须包含“仅供参考，如有不适请就医” |
...
| **Critic** | LangGraph 节点 | 读草稿 + 共享 scratchpad + 用户画像；**调用 `retrieve_safety_guidelines` 拉取 safety KB 红线**注入审核 prompt；检查健康安全、跨专家矛盾、越界建议；必要时可要求补叫 Doctor；只有真的有问题才 REVISE；审核 LLM 失败且命中风险信号时给草稿前置安全提示；每轮末尾将本轮问题/专家/回答摘要写入 `episode_store`（情节记忆写入点） |
...
| **MCP 工具降级** | wger / USDA / medical 三个社区 MCP 子进程 | `npx` 子进程起不来、stdio 握手超时、API key 无效、网络失败均会让对应 server 进入空工具列表；专家不会失败，照常用 RAG + 内置工具或通用安全知识回答 |

兜底公共逻辑集中在 `health_guide/agents/fallbacks.py`，便于后续扩展风险关键词、专家兜底话术和聚合兜底格式。

## 社区 MCP 工具接入（可选）

为 Trainer / Nutritionist / Doctor 三个角色接入了**免费、社区维护**的 MCP 工具服务器，把外部权威数据源（动作百科、USDA 食物库、FDA/PubMed/WHO/RxNorm）以 LangChain 工具的形式注入到对应专家的 ReAct 工具列表中。功能默认关闭，需要显式开启对应环境变量。
...
- **medical-mcp** 默认免认证；其中两个 puppeteer 工具（`search-google-scholar` / `search-medical-databases`）需要 Chrome 运行时库，详见下方"Doctor 用：medical-mcp"小节
...
#### Doctor 用：medical-mcp（[`medical-mcp`](https://github.com/JamesANZ/medical-mcp)，10 个工具全免认证）

> medical-mcp 聚合 FDA openFDA / PubMed E-utilities / WHO / RxNorm / Google Scholar / Cochrane / ClinicalTrials.gov 等公开医学源，对 LLM 友好统一接口。当前实现里这些工具挂在 Doctor 子 agent 下，由 Orchestrator 以 `consult_doctor` tool call 形式触发；Doctor 可按问题需要调用文献、指南和药物相关工具。
...
| 文献 | `search-medical-literature` | PubMed 文献搜索 |
...
> ```
>
> 装好后 `mcp_client.py` 会自动把 `$CONDA_PREFIX/lib` 注入到 medical-mcp 子进程的 `LD_LIBRARY_PATH`，puppeteer 下载的 Chromium 就能加载到这些 `.so`。**没装这些库时这两个工具会失败但 graceful 降级**；Doctor 仍可使用不依赖 puppeteer 的医学工具或基于通用安全知识保守回答。
>
> 注 2：medical-mcp 默认把 startup banner + 进度日志写到 stdout 污染 MCP 协议，setup 脚本会自动 sed 把它们改成 `console.error` 走 stderr。

### Doctor 调用触发条件

Orchestrator 在用户请求医学建议、症状判断、持续疼痛/不适如何处理、用药安全、处方/药物剂量、体检指标或疾病相关信息时，会调用 `consult_doctor`。Doctor 被调用后，最终回答会保留医学边界，并附上“仅供参考，如有不适请就医”。
...
| `expected_critic_verdict` | 允许的 verdict band，支持 `PASS`/`REVISE` 等前缀语义匹配 |
...
| personalization | 3.667 |  | psychologist | 4.60 |
...
`eval/safety_eval_dataset.jsonl`：29 条样本，每条带 `risk_level ∈ {none, low, medium, high}` 与 `expected_critic_verdict`，专门衡量 Doctor 医学边界 + Critic + Safety KB 的安全质量。
...
- **Doctor 触发率**：药物/症状 case 中 `Doctor` 是否被执行，以及最终回答是否保留医学免责声明
...
- `psychologist`：WHO《Mental health: strengthening our response》
...
说明：寒暄、感谢、告别和画像记录由 Orchestrator 直接处理，不走 RAG；医学边界/症状/用药类问题由 Doctor 子 agent 处理；其他专业子 agent 和 Critic safety 审核按需使用知识库。
...
- `agent`：路由到的 RAG 命名空间（`trainer` / `nutritionist` / `psychologist` / `safety`）
...
- `knowledge_base/psychologist/` — 睡眠/压力/情绪/慢病预防
...
└── health_guide/
    ...
    └── agents/
        ...
        ├── orchestrator.py          # 主 agent：直接答复普通对话；专业请求才派 Trainer/Nutritionist/Psychologist
        ├── planner.py               # 兼容别名：导出 orchestrator_node/planner_node
        ├── dispatcher.py            # 批量并行调用专家 callable + 隔离上下文 + replan 路由 + cap
        ├── trainer.py / nutritionist.py / psychologist.py   # 不再是图节点；导出 run_*() 由 Dispatcher 调用，RAG 工具按需
