...
def _deterministic_trainer_answer(pctx: dict, user_question: str) -> str:
    ...
    if injuries and any("膝关节炎" in injury or "膝" in injury for injury in injuries) and re.search(r"走多少步|多少步|步数", q):
        return (
            ...
            "若 1-2 周都没有不适，再小幅增加 500-1000 步，整个过程循序渐进。"
        )
    ...
    if re.search(r"可调哑铃|哑铃.*在家|在家.*哑铃|家练全身|全身.*一周", q):
        return (
            f"{anchor_text}只有一对可调哑铃时，先用每周 3 次全身训练最划算，隔天练，每次 40-55 分钟，强度 RPE 6-7。\n\n"
            "每次按复合动作优先：哑铃杯式深蹲 3组x8-12次、哑铃罗马尼亚硬拉/髋铰链 3组x8-12次、"
            "地板卧推或俯卧撑 3组x8-12次、单臂哑铃划船 3组x10-12次/侧、农夫走或平板支撑 2-3组。"
            "如果还有余力，再加侧平举或弯举各 2组即可。\n\n"
            "进阶规则：同一重量能稳定做到目标次数上限且动作不变形，下次每只哑铃加 1-2kg；否则先加次数。"
        )
